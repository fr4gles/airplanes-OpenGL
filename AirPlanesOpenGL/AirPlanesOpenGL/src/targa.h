...
GLboolean save_targa (const char *filename, GLsizei width, GLsizei height,GLenum format, GLenum type, GLvoid *pixels)
{
 ...
 if (format!=GL_BGR_EXT && format!=GL_BGRA_EXT && format!=GL_LUMINANCE) return GL_FALSE;
 ...
 if (format==GL_BGR_EXT || format==GL_BGRA_EXT) header [2]=TARGA_UNCOMP_RGB_IMG;
 ...
 if (format==GL_BGRA_EXT) header[16]=32;
  else if (format==GL_BGR_EXT) header[16]=24;
        else if (format==GL_LUMINANCE) header[16]=8;
 fwrite (header,TARGA_HEADER_SIZE,1,tga);
 if (format==GL_BGRA_EXT) fwrite(pixels,width*height*4,1,tga);
  else if (format==GL_BGR_EXT) fwrite (pixels,width*height*3,1,tga);
